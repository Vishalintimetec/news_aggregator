...
class ReportService:
    ...
    def report_article(self, article_id, user_id, reason):
        ...
        if report_count is None:
            raise ReportNotFoundException(f"No report found for article {article_id}")
    ...
    def get_reported_articles(self):
        reports = self.report_manager.get_reported_articles()
        if not reports:
            raise ReportNotFoundException("No reported articles found")
        return reports
