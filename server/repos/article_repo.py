...
class ArticleRepository:
    ...
    #     cursor = conn.cursor(dictionary=True)
    #     if category:
    #         cursor.execute("""SELECT * FROM news_articles WHERE published_at BETWEEN %s AND %s AND category = %s""", (start, end, category))
    #     else:
    #         cursor.execute("SELECT * FROM news_articles WHERE published_at BETWEEN %s AND %s", (start, end))
    #     result = cursor.fetchall()
    #     cursor.close()
    #     conn.close()
    #     return result
    ...
    #     conn = get_db_connection()
    #     cursor = conn.cursor(dictionary=True)
    #     cursor.execute("""SELECT na.* FROM saved_articles sa JOIN news_articles na ON sa.article_id = na.article_id WHERE sa.user_id = %s""", (user_id,))
    #     result = cursor.fetchall()
    #     cursor.close()
    #     conn.close()
    #     return result
    ...
    #     conn = get_db_connection()
    #     cursor = conn.cursor(dictionary=True)
    #     sql = """SELECT * FROM news_articles WHERE (title LIKE %s OR description LIKE %s)"""
